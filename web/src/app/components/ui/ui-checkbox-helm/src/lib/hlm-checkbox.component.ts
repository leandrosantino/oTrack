...
@Component({
	...
	template: `
		<brn-checkbox
			...
			(touched)="_onTouched?.()"
		>
			<ng-icon [class]="_computedIconClass()" hlm size="14px" name="lucideCheck" />
		</brn-checkbox>
	`,
	host: {
		class: 'contents',
...
export class HlmCheckboxComponent implements ControlValueAccessor {
	...
	protected readonly _computedClass = computed(() =>
		hlm(
			'group inline-flex border w-4 h-4 border-primary/80 shrink-0 cursor-pointer items-center rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring' +
